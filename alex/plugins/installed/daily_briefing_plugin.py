...
if configured, today's calendar events and pending tasks) and pushes it as
...
Calendar/tasks reuse the SAME credentials as google_calendar_plugin.py /
google_tasks_plugin.py (ALEX_GOOGLE_CALENDAR_*, ALEX_GOOGLE_TASKS_*) via
the shared alex.plugins.google_oauth token exchange, but built as their
own minimal client here rather than importing those plugin modules -
plugins don't depend on each other, only on shared library code. Missing
credentials just mean that section is silently skipped, not an error -
news alone is enough to satisfy "tell me the morning news" out of the box.
"""
...
async def build_briefing_text(settings) -> str:
    news, events, tasks = await asyncio.gather(
...
class GetDailyBriefingTool(Tool):
    ...
    description = (
        "Genera el briefing del dia (noticias + agenda de hoy + tareas pendientes, segun este "
        "configurado) al momento, sin esperar a la hora programada. Usa esto cuando el usuario "
        "pida las noticias, un resumen del dia, o similar."
    )
